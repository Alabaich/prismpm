...
<main class="custom-news-page">
    <div class="news-header">
        <h1>
            <?php
            if (is_tag()) {
                single_tag_title('News with tag: ');
            } else {
                echo 'All News';
            }
            ?>
        ...
        <div class="news-tag-filter">
            ...
            if ($tags) :
                ...
                $all_class = is_home() || is_front_page() ? 'active' : '';
                echo '<li><a href="' . esc_url(get_permalink(get_option('page_for_posts'))) . '" class="' . $all_class . '">All</a></li>';

                foreach ($tags as $tag) {
                    $active_class = is_tag($tag->term_id) ? 'active-tag' : '';
